...
class TakkanStoreConfig extends GroupConfig {
  TakkanStoreConfig({required super.data, required super.parent})
      : super(name: AppConfig.keyTakkanStore) {
    validate();
  }
  ...
  InstanceConfig _instanceConfig(String segment) {
    ...
    if (segmentInstanceData == null) {
      final String msg =
          "a '$segment' or 'all' element must be defined in ${AppConfig.keyTakkanStore}";
  ...
  void validate() {
    if (data.containsKey('all')) {
      const String msg =
          "${AppConfig.keyTakkanStore} should contain either 'script' and 'schema' elements or an 'all' element, but not both  This data contains both.";
